 ...
 * documents.
 */
public class AsymetricScalarQuantizedFloatVectorSimilarityValuesSource extends DoubleValuesSource {
   ...
   */
  public AsymetricScalarQuantizedFloatVectorSimilarityValuesSource(
   ...
   * @param fieldName the field name of the {@link org.apache.lucene.document.KnnFloatVectorField}
   */
  public AsymetricScalarQuantizedFloatVectorSimilarityValuesSource(float[] vector, String fieldName) {
  ...
  public DoubleValues getValues(LeafReaderContext ctx, DoubleValues scores) throws IOException {
    ...
    QuantizedByteVectorValues byteVectorValues = ((Lucene99ScalarQuantizedVectorsReader.QuantizedVectorValues) vectorValues).quantizedVectorValues;
    ...
    ScalarQuantizer docQuantizer = byteVectorValues.getScalarQuantizer();
    var scalarQuantizer = new ScalarQuantizer(docQuantizer.getLowerQuantile(), docQuantizer.getUpperQuantile(), (byte) 7);
    byte[] targetQueryVector = new byte[queryVector.length];
    float queryCorrection = scalarQuantizer.quantize(queryVector, targetQueryVector, vectorSimilarityFunction);
    final KnnVectorValues.DocIndexIterator iterator = vectorValues.iterator();
    float bitCorrection = scalarQuantizer.getConstantMultiplier() * ((float)(1 << scalarQuantizer.getBits() - 1))/(1 << docQuantizer.getBits() - 1);
    return new DoubleValues() {
      @Override
      public double doubleValue() throws IOException {
        int docId = iterator.index();
        byte[] docValue = byteVectorValues.vectorValue(docId);
        float docCorrection = byteVectorValues.getScoreCorrectionConstant(docId);
        return bitCorrection * vectorSimilarityFunction.compare(docValue, targetQueryVector) + docCorrection + queryCorrection;
      }
  ...
  public boolean equals(Object obj) {
    ...
    AsymetricScalarQuantizedFloatVectorSimilarityValuesSource other =
        (AsymetricScalarQuantizedFloatVectorSimilarityValuesSource) obj;
